...
def create_new_adhoc_project_config(project, copied_config, config_class = Xcodeproj::Project::Object::XCBuildConfiguration)
  config_name = "Adhoc"
...
def create_new_adhoc_target_config(target, copied_config, config_class = Xcodeproj::Project::Object::XCBuildConfiguration)
  config_name = "Adhoc"
...
project.build_configurations.each do |cur_config|
    ...
    if cur_config.name == "Distribution"
        puts " Found Distribution config, this used to be an old format we no longer support"
        puts " Nothing to do. Exiting"
        exit
    end
    if cur_config.name == "Adhoc"
        puts " Nothing to do. Exiting"
        exit
    end
...
create_new_adhoc_project_config(project, copied_config)
...
project.targets.each do |native_target|
    ...
    create_new_adhoc_target_config(native_target, copied_config)
end
project.save();
puts ""
